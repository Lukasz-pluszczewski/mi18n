...
describe('i18n', () => {
  ...
  describe('translator', () => {
    ...
    it('should translate and change case', () => {
      const translator = getTranslator(state);
      expect(translator.t.capitalize('test.cases.lower')).to.be.equal('Lower case');
      expect(translator.t.titleCase('test.cases.lower')).to.be.equal('Lower Case');
      expect(translator.t.camelCase('test.cases.lower')).to.be.equal('lowerCase');
      expect(translator.t.lowerCase('test.cases.upper')).to.be.equal('uppercase');
      expect(translator.t.snakeCase('test.cases.pascal')).to.be.equal('pascal_case');
      expect(translator.t.kebabCase('test.cases.pascal')).to.be.equal('pascal-case');
      expect(translator.t.upperCase('test.cases.pascal')).to.be.equal('PASCAL CASE');
      expect(translator.t.upperFirst('test.cases.camel')).to.be.equal('CamelCase');
      expect(translator.t.toUpper('test.cases.camel')).to.be.equal('CAMELCASE');
      expect(translator.t.toLower('test.cases.camel')).to.be.equal('camelcase');
      expect(translator.t.deburr('test.diacritics')).to.be.equal('asceaeoa');
    });
  });
});
